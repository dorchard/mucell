...
main = do 
   ...
   res <- return (collect [res1, res2, res3, res4, res5])
...
go xs = let
    ...
    deviation = (foldl (+) 0.0 (map (\x -> abs (x - mean)) results)) / (length xs)
    maxDeviation = (foldl (+) 0.0 (map (\x -> abs (x - maxMean)) maxResults)) / (length xs)
    in
...
processMax' maxval length ((_, run):xs) = 
              if run==0.0 then
                 ...
                 processMax' maxval (length+run) xs
